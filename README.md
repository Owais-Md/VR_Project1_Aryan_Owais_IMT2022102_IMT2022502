...
1. **Binary Classification**: Determining whether a person in an image is wearing a face mask ("with mask") or not ("without mask") using:
   - Handcrafted features with traditional machine learning (ML) classifiers.
   - A Convolutional Neural Network (CNN).
2. **Mask Segmentation**: Identifying and delineating the mask region in images of people wearing masks using:
   - Traditional region-based segmentation techniques.
   - A U-Net deep learning model.

The project is worth 15 marks and involves a total of four tasks, with deadlines set for March 24th. The implementation leverages Python, utilizing libraries such as OpenCV, scikit-learn, TensorFlow, and PyTorch, to process images, train models, and evaluate performance.
...
1. **Face Mask Detection Dataset**:
   - **Source**: [https://github.com/chandrikadebz/Face-MaskDetection/tree/master/dataset](https://github.com/chandrikadebz/Face-MaskDetection/tree/master/dataset)
...
2. **Masked Face Segmentation Dataset (MFSD)**:
   - **Source**: [https://github.com/sadjadrz/MFSD](https://github.com/sadjadrz/MFSD)
...
- **Steps**:
  1. Load images from `MSFD.zip` (face_crop).
  2. Apply K-Means (k=3) to cluster pixels.
  3. Identify mask cluster and refine with binary closing and flood fill.
...
- **Tuned XGBoost**: 92.43% accuracy (additional experiment from notebook)
